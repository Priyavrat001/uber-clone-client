...
const WaitingForDriver = ({coordinates, fares, confirmRIdeData, newRide}) => {
  return (
    <div >
      ...
      <div className='flex items-center justify-between'>
        ...
        <div className='text-right'>
          <h2 className='tex-lg font-medium'>{confirmRIdeData?.fullname.firstname + " " + confirmRIdeData?.fullname.lastname }</h2>
          <h4 className='font-semibold text-xl -mt-1 -mb-1'>{confirmRIdeData?.vehicle.map((item)=>item.plate)}</h4>
          <p className='text-sm text-gray-600'>{confirmRIdeData?.vehicle.map((item)=>item.vehicleType)}</p>
          <p className='text-lg font-semibold'>OTP {newRide.otp}</p>
        </div>
      </div>
